...
class FinTechTrainEnv(gym.Env):
    ...
    def __init__(self, df, start_balance = 10000, transaction_fees = 0.002, min_trading_unit = 5, max_trading_count = 10, max_change = 500, **kwargs):
        '''
        初始化環境
        ...
        for _df in df:
            temp_df = _df.fillna(method='bfill').reset_index()
        ...
        # 網路input的形狀
        # 用來交易的數據集(價差(最高,最低,開盤,收盤)+交易量差+技術指標-index-時間)
        # 其它觀察的數據集(價差(最高,最低,開盤,收盤))
        # 自己的票倉(餘額　貨幣數量 購買數量 購買成本 賣出數量 賣出所得)
        # self.observation_shape = (1, len(self.df[0].columns) - 2 + (len(self.df) - 1) * 4 + 6)
    ...
    def step(self, action):
        ...
        return observation, reward, finish, {'action_mask': self.action_mask, 'asset': self.assets[len(self.assets) - 2], 'current_price': current_price}
        # return observation, reward, finish, {'asset': self.assets[len(self.assets) - 2], 'current_price': current_price }

    def render(self, mode='human'):
        print('Price: ' + str(self.df[0]['Close'].values[self.current_step + self.observation_length]))
        print('購買的數量: ' + str(self.account_history[3][-1]))
        print('賣出的數量: ' + str(self.account_history[5][-1]))
        print('Net worth: ' + str(self.assets[-1]))

    def finish(self):
        return self.assets[-1] < self.start_balance / 10 or self.current_step + self.observation_length == len(self.df[0]) - self.watch_days
    ...
    def get_reward(self):
        ...
        # print(returns)
        # annualization: 年化，目前資料小時為單位(1d=24hr 1y = 365d)
        # reward = sharpe_ratio(returns, annualization=252)
    ...
    def next_observation(self):
        ...
        observation = np.insert(observation, len(observation), scaled_history[:, -1], axis=0)

        observation = np.insert(observation, len(observation), self.signal_line, axis=0)
